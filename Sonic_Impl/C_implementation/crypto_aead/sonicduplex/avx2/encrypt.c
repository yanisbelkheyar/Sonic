  ...
  )
  {
    ...
    *clen = mlen+16;
    ...
    while(adlen>=32){
        ...
        //add_to_state(state,cur_absorbe,0,4);
        add_to_state_full(state,ad,ad_id);
        //merge the absobrtion dirrectly into the state ?
        sonic_encrypt_512_procrastinate(state,state);
    ...
    //  fprintf(stderr,"tag in enc: ");
    //for(int i=0;i<16;i++)
    //    fprintf(stderr,"%x ",c[*clen-16+i]);
    //fprintf(stderr,"\n\n");
  ...
  )
  {
    ...
    *mlen = clen - 16;
    ...
    if(clen>=1){
        ...
        //printf("\n%lx , %d \n",(uint64_t)0x80<<((7-(clen & 7))<<3),(7-(clen & 7))<<3);
        //printf("\n%lx \n",cur_absorbe[clen>>3]);
        //cur_absorbe[clen>>3] ^= (uint64_t)0x80<<((7-(clen & 7))<<3);

        //for(int i=0;i<(clen>>3)+1;i++){
        //    printf("%lx ,",cur_absorbe[i]);
        //}
        //printf("\n");
    ...
    fprintf(stderr,"tag in dec: ");
    for(int i=0;i<16;i++)
        fprintf(stderr,"%x ",tag[i]);
    fprintf(stderr,"\n\n");
