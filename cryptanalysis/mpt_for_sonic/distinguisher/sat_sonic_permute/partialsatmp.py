...
class Sonic:
    count = 0
    def __init__(self, nrounds=10, solver_name="cadical153", word_size=4, branches=32, constantbit=0):
        Sonic.count += 1
        ...
        self.cnf_file_name = f"sonic_{branches*word_size}_nr_{self.nrounds}_bit{constantbit}.cnf"
        self.result_file_name = f"result_{branches*word_size}_nr_{self.nrounds}_bit{constantbit}.txt"
    ...
    def rot_left_word(self, state, val):
        ...
        for i in range(self.word_size):
            temp[i] = state[(i+val)%self.word_size]
        return temp
    ...
    def generate_constraints(self):
        ...
        for rn in range(self.nrounds):
            ...
            for word in range(self.branches//2):
                ...
                self.word_3xor(z[2*word], self.rot_left_word(y[5*word+4],12), x_in[2*word + 1], x_middle[2*word + 1])
...
def parse_args():
    ...
    parser.add_argument("-cb", "--constantbit", default=0, type=int, help="bit kept constant\n")
...
if __name__ == '__main__':
    ...
    sonic = Sonic(nrounds=nrounds,
                solver_name=solver, word_size=blocksize//2, branches=2, constantbit=constantbit)
    ...
    if constantbit < blocksize:
        ...
        print("Constant bit bigger than blocksize")
