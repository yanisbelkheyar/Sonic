    ...
    def generate_constraints(self):
        ...
        for rn in range(self.nrounds):
            ...
            for word in range(self.branches//2):   
                ...
                self.word_3xor(self.rot_left_word(y[5*word+6],6), self.rot_left_word(y[5*word+8],15), self.rot_left_word(y[5*word+9],23), y[5*word+10])
...
if __name__ == '__main__':
    ...
    with open(sonic.result_file_name, "w") as outputfile:
        outputfile.write(f"Results of applying monomial prediction method on {sonic.nrounds} rounds of Sonic_{blocksize}\n")
    mp_dict = dict()
