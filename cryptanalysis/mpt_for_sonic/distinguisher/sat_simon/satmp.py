...
class Simon:
    ...
    def __init__(self, nrounds=10, solver_name="cadical", word_size=4, branches=32):
        ...
        self.cnf_file_name = f"simon_{branches*word_size}_nr_{self.nrounds}.cnf"
    ...
    def generate_round_y_z_k_variables(self, rn):
        ...
        y = [[f"y_{rn}_{word}_{bit}" for bit in range(self.word_size)] for word in range((self.branches//2)*5)]
        z = [[f"z_{rn}_{word}_{bit}" for bit in range(self.word_size)] for word in range(self.branches)]
    ...
    def generate_constraints(self):
        ...
        for rn in range(self.nrounds):
            ...
            for word in range(self.branches//2):   
                ...
                self.word_fork(y[5*word+3], y[5*word+4], x_middle[2*word])
                self.word_and(self.rot_left_word(y[5*word],1),self.rot_left_word(y[5*word+2],8), z[2*word])
                self.word_3xor(z[2*word], self.rot_left_word(y[5*word+4],2), x_in[2*word + 1], z[2*word+1])
                self.word_xor(z[2*word+1], k[word], x_middle[2*word + 1])
...
if __name__ == '__main__':
    ...
    with open(simon.result_file_name, "w") as outputfile:
        outputfile.write(f"Results of applying monomial prediction method on {simon.nrounds} rounds of Simon_{blocksize}\n")
    mp_dict = dict()
    simon.generate_sat_model()
    if not os.path.exists("mpdict_{branches*word_size}_nr_{self.nrounds}.pyobj"):
        start_time = time.time()
        for bit in range(0, blocksize):            
            print(separator_line)
            print(f"Fixed index: {bit}")
            mp_dict[(bit,)] = simon.check_cube(fixed_indices=[bit], target_output_bits = range(blocksize))
        elapsed_time = time.time() - start_time
        time_line = "Total time to compute monomial prediction dictionary (mpdict): {:0.02f}\n".format(elapsed_time)        
        with open(simon.result_file_name, "a") as outputfile:
    ...
    while flag:
        ...
        for fixed_indices in combinations(sufficient_list, constant_part_size):
            ...
            if common_output_bbits != {}:
                key_independent_bits = simon.check_cube(fixed_indices=list(fixed_indices), 
    ...
    print(final_result)
    with open(simon.result_file_name, "a") as output_file:
