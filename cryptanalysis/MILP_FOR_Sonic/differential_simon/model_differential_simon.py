...
def xor_3_bit_right_new(a,r,SLR1):
    ...
    P.addConstr(xp[r,a]+x[r,shiffte(a,SLR1)]+x[r,a+H_SONIC_SIZE]-y[r,a]>=0)
    P.addConstr(-xp[r,a]-x[r,shiffte(a,SLR1)]-x[r,a+H_SONIC_SIZE]+y[r,a]>=-2)
    P.addConstr(-xp[r,a]+x[r,shiffte(a,SLR1)]+x[r,a+H_SONIC_SIZE]+y[r,a]>=0)
    P.addConstr(xp[r,a]-x[r,shiffte(a,SLR1)]+x[r,a+H_SONIC_SIZE]+y[r,a]>=0)
    P.addConstr(xp[r,a]+x[r,shiffte(a,SLR1)]-x[r,a+H_SONIC_SIZE]+y[r,a]>=0)
    P.addConstr(xp[r,a]-x[r,shiffte(a,SLR1)]-x[r,a+H_SONIC_SIZE]-y[r,a]>=-2)
...
def shuffle(r):
    for i in range(H_SONIC_SIZE):
        P.addConstr(y[r,((i)%SONIC_SIZE)]==x[r,i])
...
def add_constraint(SNL1,SNL2,SLR1,min_weight,NBR_round):
...
#add the constraint for NBR_round of the round function with the rotation value specify as argument to the model P
def add_constraint_full(SNL1,SNL2,SLR1,min_weight,NBR_round):
    if(not (P.NumConstrs==0)):
        P.remove(P.getConstrs()[0:P.NumConstrs-1])
