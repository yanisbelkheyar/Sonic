from model_linear import *

#!!this function can only be call after the optimization of the model!!
#
#This function print the trail with minimum weight find after minimization of the objectiv function
def print_trail(SNL1,SNL2,SLR1,SLL1,SLL2,SLL3,M,NBR_round):
    list_a = []
    weight = P.PoolObjVal
    
    print("tot weight for "+str(NBR_round)+" = "+str(weight))
    print("poly use : X^"+str(SLL1)+"+X^"+str(SLL2)+"+X^"+str(SLL3)+"   and rotation ("+str(SNL1)+","+str(SNL2)+"), shift "+str(SLR1)+" and mul shuffle "+str(M))

    input_dif,output_dif = [0,0,0,0],[0,0,0,0]
    for i in range(H_SONIC_SIZE//2):
        for j in range(4):
            input_dif[j] += int(x[0,i+(j*(H_SONIC_SIZE//2))].getAttr('xn'))*(2**((H_SONIC_SIZE//2)-i-1))
            output_dif[j] += int(x[NBR_round,i+(j*(H_SONIC_SIZE//2))].getAttr('xn'))*(2**((H_SONIC_SIZE//2)-i-1))

    for i in range(4):
        print(hex(input_dif[i]),end=" , ")
    print()
    for i in range(4):
        print(hex(output_dif[i]),end=" , ")
    print()

    #print a trail with min weight and iterate over all trail with min weight
    for i in range(NBR_round):
        print("X"+str(i)+":",end="          ")
        #else:
        #    print("B"+str(i//2)+":",end="          ")
        #print the state corresponding to the input of the non linear layer
        for j in range(H_SONIC_SIZE):
            if(x[i,j].getAttr('xn')==1):
                print("1",end="")
                list_a.append([i,j])
            else:
                print("-",end="")
                
        print("  ",end="")
        for j in range(H_SONIC_SIZE,SONIC_SIZE):
            if(x[i,j].getAttr('xn')==1):
                print("1",end="")
                list_a.append([i,j])
            else:
                print("-",end="")
        print("\nY"+str(i)+":",end="          ")
        for j in range(H_SONIC_SIZE):
            if(y[i,j].getAttr('xn')==1):
                print("1",end="")
                list_a.append([i,j])
            else:
                print("-",end="")
                
        print("  ",end="")
        for j in range(H_SONIC_SIZE,SONIC_SIZE):
            if(y[i,j].getAttr('xn')==1):
                print("1",end="")
                list_a.append([i,j])
            else:
                print("-",end="")
                
                #print("\noutput dif:  ",end="")
                #print the state corresponding to the output of the non linear layer
                #for j in range(H_SONIC_SIZE):
                #    if(y[i,j+H_SONIC_SIZE].getAttr("x")==1):
                #        print("1",end="")
                #    else:
                #        print("-",end="")
        """
        for j in range(H_SONIC_SIZE,SONIC_SIZE):
            if(x[i,j-H_SONIC_SIZE].getAttr('x')==1):
                print()
                print("x ",j,"=1 : ")
                print(s1[i,shiffte(j-H_SONIC_SIZE,0)])
                print(s2[i,shiffte(j-H_SONIC_SIZE,1)])
                print(f[i,j-H_SONIC_SIZE])
            if(x[i,j].getAttr('x')==1):
                print()
                print("x ",j,"=1 : ")
                print(s1[i,j-H_SONIC_SIZE])
                print(s2[i,j-H_SONIC_SIZE])
            if(f[i,j-H_SONIC_SIZE].getAttr('x')==1):
                print()
                print("f ",j-H_SONIC_SIZE,"=1 :")
                print(x[i,shiffte(j-H_SONIC_SIZE,5)+H_SONIC_SIZE])
                print(e[i,j-H_SONIC_SIZE])
            if(e[i,j-H_SONIC_SIZE].getAttr('x')==1):
                print("e ",j-H_SONIC_SIZE,"=1 :")
                print(y[i,shiffte(j-H_SONIC_SIZE,-0)])
                print(y[i,shiffte(j-H_SONIC_SIZE,-7)])
                print(y[i,shiffte(j-H_SONIC_SIZE,-32)])
        """    
            
        print()

    #print("list index for active bit in x state")
    #for i in list_a:
    #    print(i,end=", ")
    #print()
    


def print_all(NBR_round):
    print()
    for i in range(NBR_round):
        print("X",i,end="  ")
        for j in range(H_SONIC_SIZE):
                if(x[i,j].getAttr('xn')==1):
                    print("1",end="")
                else:
                    print("-",end="")
        print("  ",end="")
        for j in range(H_SONIC_SIZE,SONIC_SIZE):
            if(x[i,j].getAttr('xn')==1):
                print("1",end="")
            else:
                print("-",end="")
        print()
        print("A",i,end="  ")
        for j in range(H_SONIC_SIZE):
            print(" ",end="")
        print("  ",end="")
        for j in range(H_SONIC_SIZE):
            if(A[i,j].getAttr('xn')==1):
                print("1",end="")
            else:
                print("-",end="")
        print()
        print("xp",i,end=" ")
        for j in range(H_SONIC_SIZE):
            if(xp[i,j].getAttr('xn')==1):
                print("1",end="")
            else:
                print("-",end="")
        print()
        print("f ",i,end=" ")
        for j in range(H_SONIC_SIZE):
            if(f[i,j].getAttr('xn')==1):
                print("1",end="")
            else:
                print("-",end="")
        print()
        print("e ",i,end=" ")
        for j in range(H_SONIC_SIZE):
            if(e[i,j].getAttr('xn')==1):
                print("1",end="")
            else:
                print("-",end="")
        print("  ",end="")
        print()
        print("Y",i,end="  ")
        for j in range(H_SONIC_SIZE):
                if(y[i,j].getAttr('xn')==1):
                    print("1",end="")
                else:
                    print("-",end="")
        print("  ",end="")
        for j in range(H_SONIC_SIZE,SONIC_SIZE):
            if(y[i,j].getAttr('xn')==1):
                print("1",end="")
            else:
                print("-",end="")
        print()
        print("Z",i,end="  ")
        for j in range(SONIC_SIZE):
            if(j==H_SONIC_SIZE):
                print("  ",end="")
            if(z[i,(j+H_SONIC_SIZE)%SONIC_SIZE].getAttr('xn')==1):
                print("1",end="")
            else:
                print("-",end="")
        print()
        print()
    print()
    print()
