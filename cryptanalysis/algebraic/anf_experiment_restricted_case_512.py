...
state0_right = [0 for i in range(H_STATE_SIZE)]#[bit_variables[i+H_STATE_SIZE] for i in range(H_STATE_SIZE)]
...
def theta(state_bits):
    return [state_bits[i] + state_bits[(i+_sage_const_7 ) % H_STATE_SIZE] + state_bits[(i+25 ) % H_STATE_SIZE] for i in range(H_STATE_SIZE)]
